...
in `procurement.po_drafts` with status DRAFT. The budget_gate agent then
    ...
    return db.fetchall(
        ...
        """WITH latest AS (
           ...
           SELECT l.*, o.sku AS out_sku, o.supplier_id AS out_supplier
             FROM latest l
             JOIN procurement.email_outbox o
               ON o.thread_id = l.thread_id
              AND o.intent = 'RFQ'
            WHERE NOT EXISTS (
                    SELECT 1 FROM procurement.po_drafts p
...
def run_po_drafter(settings: Settings) -> dict:
    ...
    for row in _candidate_threads(settings):
        ...
        db.execute(
            settings,
            """INSERT INTO procurement.po_drafts
