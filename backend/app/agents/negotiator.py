...
shows). When a supplier reply lands in `procurement.email_inbox`, the agent
...
def _draft_rfq(
    ...
    db.execute(
        settings,
        """INSERT INTO procurement.email_outbox
...
def _unprocessed_inbound(settings: Settings) -> list[dict]:
    return db.fetchall(
        settings,
        """SELECT * FROM procurement.email_inbox
...
def _process_inbound(
    ...
    try:
        ...
        db.execute(
            settings,
            "UPDATE procurement.email_inbox SET processed=TRUE WHERE email_id=%s",
            [email["email_id"]],
        )
        return
    ...
    db.execute(
        settings,
        """UPDATE procurement.email_inbox
...
def simulate_supplier_reply(
    ...
    thread = db.fetchone(
        settings,
        """SELECT * FROM procurement.email_outbox
